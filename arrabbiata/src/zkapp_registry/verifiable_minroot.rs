...
use super::{verifier::Verifier, ZkApp};
use crate::{
    curve::ArrabbiataCurve,
    interpreter::InterpreterEnv,
    zkapp_registry::{VerifiableZkApp, VerifierApp},
};
use ark_ff::PrimeField;

#[derive(Copy, Clone)]
pub enum Instruction<C, Verifier>
where
    C: ArrabbiataCurve,
    C::BaseField: PrimeField,
    Verifier: VerifierApp<C>,
{
    ComputeFifthRoot(usize),
    Verify(Verifier::Instruction),
}

#[derive(Eq, Hash, PartialEq)]
pub enum Gadget<C, Verifier>
where
    C: ArrabbiataCurve,
    C::BaseField: PrimeField,
    Verifier: VerifierApp<C>,
{
    ComputeFifthRoot,
    Verify(Verifier::Gadget),
}

impl<C, Verifier> From<Instruction<C, Verifier>> for Gadget<C, Verifier>
where
    C: ArrabbiataCurve,
    C::BaseField: PrimeField,
    Verifier: VerifierApp<C>,
{
    fn from(instr: Instruction<C, Verifier>) -> Gadget<C, Verifier> {
        match instr {
            Instruction::ComputeFifthRoot(_) => Gadget::ComputeFifthRoot,
            Instruction::Verify(instr) => Gadget::Verify(instr.into()),
        }
    }
}
...
where
    ...
    C::BaseField: PrimeField,
{
    pub fn new(x: C::ScalarField, y: C::ScalarField, n: usize) -> Self {
        Self {
            _x: x,
            _y: y,
            n,
...
where
    ...
    C::BaseField: PrimeField,
{
    type Instruction = Instruction<C, Verifier<C>>;
    type Gadget = Gadget<C, Verifier<C>>;
        ...
        match current_instr {
            Instruction::ComputeFifthRoot(i) => {
                if i < self.n - 1 {
                    ...
                    Some(Instruction::Verify(self.verifier.fetch_instruction()))
                }
            }
            Instruction::Verify(instr) => self
                .verifier
                .fetch_next_instruction(instr)
                .map(Instruction::Verify),
        }
    }
    ...
    fn run<E: InterpreterEnv>(&self, env: &mut E, instr: Self::Instruction) {
        match instr {
            ...
            Instruction::Verify(instr) => self.verifier.run(env, instr),
        }
    }
}

impl<C> VerifiableZkApp<C> for MinRoot<C>
where
    C: ArrabbiataCurve,
    C::BaseField: PrimeField,
{
