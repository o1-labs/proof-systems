...
pub fn bench_proof_creation(c: &mut Criterion) {
    ...
    group.bench_function(
        format!("proof creation small (SRS size 2^{})", ctx.srs_size()),
        |b| b.iter(|| black_box(ctx.create_proof())),
    );

    let ctx = BenchmarkCtx::new(14);
    group.bench_function(
        format!("proof creation big (SRS size 2^{})", ctx.srs_size()),
        |b| b.iter(|| black_box(ctx.create_proof())),
    );
    ...
    group.bench_function(
        format!("proof verification big (SRS size 2^{})", ctx.srs_size()),
