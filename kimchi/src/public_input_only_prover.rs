//! This module implements a prover specialized to generate a proof
//! where only the public input is considered. Intended for testing.
//!
//! This is done by using the generic gate with only first coefficient
//! enabled. All other gates are disabled, as well as zero-knowledge,
//! and the permutation argument is hard-coded as the identity
//! function, to minimize the overhead of proving.
//!
//! Proofs generated by this module are explicitly designed to be
//! compatible with the kimchi verifier, and hence the pickles
//! verifier circuit.
...
type ProverResult<T> = std::result::Result<T, ProverError>;

/// Creates a hard-coded verifier index that proofs generated by
...
pub fn verifier_index<G: KimchiCurve>(
    ...
    // FeatureFlags with all features disabled.
    let feature_flags: FeatureFlags = std::default::Default::default();
    ...
    G::BaseField: PrimeField,
{
    ...
    >(
        ...
        Self::create_recursive_public_input_only::<EFqSponge, EFrSponge, RNG>(
            groupmap,
            witness,
            index,
            Vec::new(),
...
fn test_public_input_only_prover() {
    ...
    eprintln!("- time to create srs: {:?}ms", start.elapsed().as_millis());
    ...
    let proof = ProverProof::create_recursive_public_input_only::<BaseSponge, ScalarSponge, _>(
        &group_map,
        public_inputs.clone(),
        &verifier_index,
        vec![],
    ...
    // Checking validity of the proof with the actual Kimchi verifier.
    let start = Instant::now();
    verify::<Pallas, BaseSponge, ScalarSponge>(&group_map, &verifier_index, &proof, &public_inputs)
        .unwrap();
    eprintln!("- time to verify: {}ms", start.elapsed().as_millis());
}
