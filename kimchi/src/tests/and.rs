...
fn test_and(in1: PallasField, in2: PallasField, bytes: usize) -> [Vec<PallasField>; COLUMNS] {
...
fn test_and8_random() {
    let bytes = 1;
    let input1 = PallasField::random(bytes * 8);
    let input2 = PallasField::random(bytes * 8);
    let witness = test_and(input1, input2, bytes);
    check_and(&witness, bytes, input1, input2);
}
