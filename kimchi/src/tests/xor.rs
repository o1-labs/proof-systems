...
fn test_prove_and_verify_not_xor() {
    let bits = 64;
    // Create
    let (mut next_row, mut gates) = CircuitGate::<Fp>::create_not_gadget(0, bits);
    ...
    // Create witness and random inputs
    let witness = xor::create_not_witness(&big_random(bits), Some(bits));
...
fn test_prove_and_verify_not_gnrc() {
    let bits = 64;
    // Create
    let (mut next_row, mut gates) = CircuitGate::<Fp>::create_not_gnrc(false);
    ...
    // Create witness and random inputs
    let witness = xor::create_not_gnrc_witness(&big_random(bits), &None, bits);
