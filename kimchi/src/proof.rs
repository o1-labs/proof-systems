...
impl<Eval, const COLUMNS: usize> ProofEvaluations<Eval, COLUMNS> {
    pub fn map<Eval2, FN: Fn(Eval) -> Eval2>(self, f: &FN) -> ProofEvaluations<Eval2, COLUMNS> {
        ...
        ProofEvaluations {
            ...
            keccak_round0_selector: keccak_round0_selector.map(f),
            keccak_round1_selector: keccak_round1_selector.map(f),
    ...
    pub fn map_ref<Eval2, FN: Fn(&Eval) -> Eval2>(
        ...
        ProofEvaluations {
            ...
            keccak_round0_selector: keccak_round0_selector.as_ref().map(f),
            keccak_round1_selector: keccak_round1_selector.as_ref().map(f),
...
impl<F: Zero + Copy, const COLUMNS: usize> ProofEvaluations<PointEvaluations<F>, COLUMNS> {
    pub fn dummy_with_witness_evaluations(
        ...
        ProofEvaluations {
            ...
            xor_selector: None,
            rot_selector: None,
            keccak_round0_selector: None,
            keccak_round1_selector: None,
...
impl<F, const COLUMNS: usize> ProofEvaluations<F, COLUMNS> {
    pub fn get_column(&self, col: Column) -> Option<&F> {
        match col {
            ...
            Column::Index(GateType::KeccakRound0) => self.keccak_round0_selector.as_ref(),
            Column::Index(GateType::KeccakRound1) => self.keccak_round1_selector.as_ref(),
