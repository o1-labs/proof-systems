//! This module implements short Weierstrass curve variable base scalar
//! multiplication custom Plonk polynomials.
//!
//! ```ignore
//! Acc := [2]T
//! for i = n-1 ... 0:
...
//~ We implement custom Plonk constraints for short Weierstrass curve variable
//~ base scalar multiplication.
//~
//~ Given a finite field $\mathbb{F}_q$ of order $q$, if the order is not a
//~ multiple of 2 nor 3, then an elliptic curve over $\mathbb{F}_q$ in short
//~ Weierstrass form is represented by the set of points $(x,y)$ that satisfy
//~ the following equation with $a,b\in\mathbb{F}_q$ and
//~ $4a^3+27b^2\neq_{\mathbb{F}_q} 0$: $$E(\mathbb{F}_q): y^2 = x^3 + a x + b$$
//~ If $P=(x_p, y_p)$ and $Q=(x_q, y_q)$ are two points in the curve
//~ $E(\mathbb{F}_q)$, the algorithm we represent here computes the operation
//~ $2P+Q$ (point doubling and point addition) as $(P+Q)+Q$.
//~
//~ ```admonish info
...
//~ which can perform the above operation using 1 multiplication, 2 squarings
//~ and 2 divisions (one more squaring) if $P=Q$), thanks to the fact that
//~ computing the $Y$-coordinate of the intermediate addition is not required.
//~ This is more efficient to the standard algorithm that requires 1 more
//~ multiplication, 3 squarings in total and 2 divisions.
//~
//~ Moreover, this algorithm can be applied not only to the operation $2P+Q$,
//~ but any other scalar multiplication $kP$. This can be done by expressing the
//~ scalar $k$ in biwise form and performing a double-and-add approach.
//~ Nonetheless, this requires conditionals to differentiate $2P$ from $2P+Q$.
//~ For that reason, we will implement the following pseudocode from <https://github.com/zcash/zcash/issues/3924> (where instead, they give a variant
//~ of the above efficient algorithm for Montgomery curves $b\cdot y^2 = x^3 + a
//~ \cdot x^2 + x$).
//~
//~ ```ignore
//~ Acc := [2]T
...
//~ The i-th row will be a `VBSM` gate whereas the next row will be a `ZERO`
//~ gate.
//~
...
//~ Then `Output` will be the point with coordinates `(xO, yO)` resulting from
//~ `O = ( I ± T ) + I`
//~
//~ ```admonish info
...
//~ ```
//~
//~ In each step of the algorithm, we consider the following elliptic curves
//~ affine arithmetic equations:
//~
...
//~ * `u` $:= 2 \cdot y_i - $ `t` $\cdot s_1 \iff 2 \cdot y_i - s_1 \cdot
//~   (2\cdot x_i - s^2_1 + x_t)$
//~
//~ Next, for each bit in the algorithm, we create the following 4 constraints
//~ that derive from the above:
//~
...
//~
//~ Finally, the additional constraint makes sure that the scalar is being
//~ correctly expressed into its binary form (using the double-and-add
//~ decomposition) as: $$ n' = 2^5 \cdot n + 2^4 \cdot b_0 + 2^3 \cdot b_1 + 2^2
//~ \cdot b_2 + 2^1 \cdot b_3 + b_4$$ This equation is translated as the
//~ constraint:
//~
