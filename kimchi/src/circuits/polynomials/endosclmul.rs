...
//~ the following equation with $a,b\in\mathbb{F}_{q}$
//~ and
//~ $4a^3+27b^2\neq_{\mathbb{F}_q} 0 $:
//~ $$E(\mathbb{F}_q): y^2 = x^3 + a x + b$$
//~ If $P=(x_p, y_p)$ and $T=(x_t, y_t)$ are two points in the curve
//~ $E(\mathbb{F}_q)$, the goal of this operation is to perform the operation
//~ $2P±T$ efficiently as $(P±T)+P$.
//~
//~ `S = (P + (b ? T : −T)) + P`
//~
//~ The same algorithm can be used to perform other scalar multiplications,
//~ meaning it is not restricted to the case $2\cdot P$, but it can be used for
//~ any arbitrary $k\cdot P$. This is done by decomposing the scalar $k$ into
//~ its binary representation. Moreover, for every step, there will be a one-bit
//~ constraint meant to differentiate between addition and subtraction
...
//~ inputs of the following row, becoming $P$ in the next constraints.
//~ Similarly, the scalar is decomposed into binary form and $n$ ($n'$
//~ respectively) will store the current accumulated value and the next one for
//~ the check.
//~
...
//~ * First block:
//~   * `(xq1 - xp) * s1 = yq1 - yp`
//~   * `(2 * xp – s1^2 + xq1) * ((xp – xr) * s1 + yr + yp) = (xp – xr) * 2 *
//~     yp`
...
//~   * <=> $(2 \cdot x_p – s_1^2 + x_{q_1}) \cdot ((x_p – x_r) \cdot s_1 + y_r
//~     + y_p) = (x_p – x_r) \cdot 2 \cdot y_p$
...
//~   * <=> $(2 \cdot x_r – s_3^2 + x_{q_2}) \cdot ((x_r – x_s) \cdot s_3 + y_s
//~     + y_r) = (x_r – x_s) \cdot 2 \cdot y_r$
...
//~
//~ Gives the following equations when substituting the values of $s_2$ and
//~ $s_4$:
//~
...
impl<F: PrimeField> CircuitGate<F> {
    ...
    /// # Errors
    ///
    /// Will give error if `self.typ` is not `GateType::EndoMul`, or `constraint
    /// evaluation` fails.
