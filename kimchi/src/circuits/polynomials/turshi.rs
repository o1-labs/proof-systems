    ...
    F: PrimeField,
{
    ...
    fn constraint_checks(env: &ArgumentEnv<F, E<F>>, cache: &mut Cache) -> Vec<E<F>> {
        ...
        let op1_src = cache.cache::<F, BerkeleyChallengeTerm>(
            f_op1_ap.clone() + f_op1_fp.clone() + f_op1_val.clone(),
        );
        let res_log =
            cache.cache::<F, BerkeleyChallengeTerm>(f_res_mul.clone() + f_res_add.clone());
        ...
        let opcode = cache
            .cache::<F, BerkeleyChallengeTerm>(f_opc_aeq.clone() + f_opc_ret + f_opc_call.clone());
