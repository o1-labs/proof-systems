//! This module includes the definition of the NOT gadget and the witness code
//! generation, for both the implementation running with `Xor16` gates and the
//! one with `Generic` gates. Note that this module does not include a `Not`
//! gate type.
use crate::circuits::{
...
//~ observation: *the bitwise NOT operation is equivalent to the bitwise XOR
//~ operation with the all one words of a certain length*. In other words,
//~ $$\neg x = x \oplus 1^*$$
//~ where $1^*$ denotes a bitstring of all ones of length $|x|$. Let $x_i$ be
//~ the $i$-th bit of $x$, the intuition is that if $x_i = 0$ then XOR with $1$
//~ outputs $1$, thus negating $x_i$. Similarly, if $x_i = 1$ then XOR with 1
//~ outputs 0, again negating $x_i$. Thus, bitwise XOR  with $1^*$ is equivalent
//~ to bitwise negation (i.e. NOT).
//~
//~ Then, if we take the XOR gadget with a second input to be the all one word
//~ of the same length, that gives us the NOT gadget.  The correct length can be
//~ imposed by having a public input containing the `2^bits - 1` value and
//~ wiring it to the second input of the XOR gate. This approach needs as many
//~ rows as an XOR would need, for a single negation, but it comes with the
//~ advantage of making sure the input is of a certain length.
//~
//~ The other approach can be more efficient if we already know the length of
//~ the inputs. For example, the input may be the input of a range check gate,
//~ or the output of a previous XOR gadget (which will be the case in our Keccak
//~ usecase). In this case, we simply perform the negation as a subtraction of
//~ the input word from the all one word (which again can be copied from a
//~ public input). This comes with the advantage of holding up to 2 word
//~ negations per row (an eight-times improvement over the XOR approach), but it
//~ requires the user to know the length of the input.
//~
//~ ##### NOT Layout using XOR
//~
//~ Here we show the layout of the NOT gadget using the XOR approach. The gadget
//~ needs a row with a public input containing the all-one word of the given
//~ length. Then, a number of XORs follow, and a final `Zero` row is needed. In
//~ this case, the NOT gadget needs $\ceil(\frac{|x|}{16})$ `Xor16` gates, that
//~ means one XOR row for every 16 bits of the input word.
//~
...
//~
//~ Here we show the layout of the NOT gadget using the Generic approach. The
//~ gadget needs a row with a public input containing the all-one word of the
//~ given length, exactly as above. Then, one Generic gate reusing the all-one
//~ word as left inputs can be used to negate up to two words per row. This
//~ approach requires that the input word is known (or constrained)
...
impl<F: PrimeField> CircuitGate<F> {
    ...
    /// previously constrained using a generic gate (checking that a cell
    /// stores `2^bits-1` value). Assumes that the inputs are known to have at
    /// most `bits` length. Starts the gates in the `new_row` position.
    /// Includes:
    ...
    /// - pub_row   : row containing the public input with the all-one word of
    ///   the given length
    /// Important:
    ...
    // - pub_row        : row where the public inputs is stored (the 2^bits - 1)
    //   value (in the column 0 of that row)
    // - double_generic : whether to perform two NOTs or only one inside the generic
    //   gate
    // Output:
    // - new_row : next row after the double NOT generic gate, corresponds to
    //   `new_row+1`
    fn extend_not_gnrc(
        gates: &mut Vec<Self>,
        pub_row: usize,
        ...
        // check left inputs of the double generic gate correspond to the 2^bits - 1
        // value
    ...
    /// It implicitly constrains the length of the input to be at most 16 *
    /// num_xors bits. Includes:
    /// - num_xors Xor16 gates
    /// - 1 Generic gate to constrain the final row to be zero with itself
    /// Input:
    /// - gates : full circuit
    /// - pub_row : row containing the public input with the all-one word of the
    ///   given length
    /// - bits    : number of bits of the input
    /// Precndition:
    /// - 1 initial public input generic gate in `all_ones_row` to constrain the
    ///   input to be `2^bits-1`.
    /// Warning:
    /// - don't forget to connect the left input to a public input row
    ///   containing the `2^bits - 1` value
...
/// `input` in bits. If `bits` is provided, the negation takes the maximum
/// length between `bits` and that of `input`. Warning:
/// - don't forget to set a row of the witness with public input `2^bits -1` and
///   wire it to the second input of the first `Xor16` gate
...
/// Extends negation witnesses from generic gate, assuming the input witness
/// already contains public input rows holding the 2^bits-1 value.
/// Input: a vector of words to be negated, and the number of bits (all the
/// same) Returns error if the bits length is too small for the inputs
