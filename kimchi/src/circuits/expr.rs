...
impl Variable {
    fn evaluate<'a, 'b, F: Field>(&self, evals: &'a [ProofEvaluations<F>]) -> Result<F, &'b str> {
        ...
        match self.col {
            ...
            Index(GateType::CairoClaim) => Ok(evals.cairo_selector[0]),
            Index(GateType::CairoInstruction) => Ok(evals.cairo_selector[1]),
            Index(GateType::CairoFlags) => Ok(evals.cairo_selector[2]),
            Index(GateType::CairoTransition) => Ok(evals.cairo_selector[3]),
