...
/// Compute the `x`th evaluations of the unnormalized lagrange polynomial on
/// H_8. Taking H_8 as an example, we show how to compute this
...
impl<F: FftField, Column: PartialEq + Copy + Sync, ChallengeTerm: Copy + Sync>
    Expr<ConstantExpr<F, ChallengeTerm>, Column>
{
    ...
    pub fn evaluations<
        'a,
        Challenges: Index<ChallengeTerm, Output = F> + Sync,
        Environment: ColumnEnvironment<'a, F, ChallengeTerm, Challenges, Column = Column> + Sync,
    >(
        &self,
        env: &'a Environment,
    ) -> Evaluations<F, D<F>> {
...
impl<
        ...
        'b,
        F: FftField,
        ChallengeTerm: Copy + PartialEq + Sync,
        Challenges: Index<ChallengeTerm, Output = F> + Sync,
        Column: PartialEq + Copy + Sync,
        Environment: ColumnEnvironment<'a, F, ChallengeTerm, Challenges, Column = Column> + Sync,
...
impl<F: FftField, Column: Copy + PartialEq + Copy + Sync> Expr<F, Column> {
...
impl<F: FftField, Column: Debug + PartialEq + Copy + Sync, ChallengeTerm: Copy + Sync>
    Linearization<Expr<ConstantExpr<F, ChallengeTerm>, Column>, Column>
{
    /// Given a linearization and an environment, compute the polynomial corresponding to the
    /// linearization, in evaluation form.
