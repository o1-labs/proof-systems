...
//~ The range check table is a single-column table containing the numbers from 0
//~ to 2^12 (excluded). This is used to check that the value fits in 12 bits.

use crate::circuits::lookup::tables::{LookupTable, RANGE_CHECK_TABLE_ID};
use ark_ff::Field;

/// The range check will be performed on 12-bit values, i.e. those in `[0,
/// 2^12)`
pub const RANGE_CHECK_UPPERBOUND: u32 = 1 << 12;

/// A single-column table containing the numbers from 0 to
/// [`RANGE_CHECK_UPPERBOUND`] (exclusive)
