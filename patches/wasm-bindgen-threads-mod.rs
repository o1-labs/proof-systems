...
// --------------------------------------------------------------------------
// Memory64-aware address helpers
// --------------------------------------------------------------------------

/// Push an address constant onto the wasm stack.
fn push_addr(body: &mut InstrSeqBuilder<'_>, val: i64, memory64: bool) {
    if memory64 {
        body.i64_const(val);
    } else {
...
/// Push a size/align constant onto the wasm stack (same width as addresses).
fn push_usize(body: &mut InstrSeqBuilder<'_>, val: u64, memory64: bool) {
    if memory64 {
...
fn addr_ty(memory64: bool) -> ValType {
    if memory64 {
        ValType::I64
    } else {
        ValType::I32
...
fn addr_add(memory64: bool) -> walrus::ir::BinaryOp {
    if memory64 {
...
fn addr_zero_value(memory64: bool) -> Value {
    if memory64 {
        Value::I64(0)
    } else {
        Value::I32(0)
...
fn addr_value(val: i64, memory64: bool) -> Value {
    if memory64 {
        Value::I64(val)
    } else {
...
/// Convert an addr-typed value on the stack to i32 for use as an `if` condition.
/// In wasm, `if` always expects i32. For memory64, we compare != 0 to get i32.
fn addr_to_bool(body: &mut InstrSeqBuilder<'_>, memory64: bool) {
    if memory64 {
        body.i64_const(0);
        body.binop(walrus::ir::BinaryOp::I64Ne);
    }
    // For wasm32, the value is already i32, no conversion needed.
}
...
pub fn run(module: &mut Module) -> Result<Option<ThreadCount>, Error> {
    ...
    let thread_counter_addr = addr as i64;

    let stack_alloc = module.globals.add_local(
        addr_ty(memory64),
        true,
        false,
        ConstExpr::Value(addr_zero_value(memory64)),
    );

    // Make sure the temporary stack is aligned down
    let temp_stack = (base + static_data_pages as u64 * PAGE_SIZE as u64)
        & !(static_data_align as u64 - 1);
    ...
    let stack = Stack {
        ...
        size: module.globals.add_local(
            ...
            true,
            false,
            ConstExpr::Value(addr_value(DEFAULT_THREAD_STACK_SIZE as i64, memory64)),
        ),
    };
    ...
    let thread_count =
        inject_start(module, &tls, &stack, thread_counter_addr, memory, memory64)?;
...
fn delete_synthetic_global(module: &mut Module, name: &str) -> Result<u64, Error> {
    ...
    match g {
        ConstExpr::Value(Value::I32(v)) => Ok(*v as u32 as u64),
...
fn allocate_static_data(
    ...
    // here to make sure that `__heap_base` is a non-mutable integer, and then do
    // some logic to ensure that we return the correct, aligned `address` as specified
    // by `align`.
    let (base, address) = {
        ...
        match &mut global.kind {
            GlobalKind::Local(ConstExpr::Value(Value::I32(offset))) => {
                let address =
                    (*offset as u32 + (align - 1)) & !(align - 1);
                let base = *offset as u64;
                *offset += (pages * PAGE_SIZE) as i32;
                (base, address as u64)
            }
            GlobalKind::Local(ConstExpr::Value(Value::I64(offset))) => {
                let address =
                    (*offset as u64 + (align as u64 - 1)) & !(align as u64 - 1);
                let base = *offset as u64;
                *offset += pages as i64 * PAGE_SIZE as i64;
                (base, address)
            }
            _ => bail!("`__heap_base` not a locally defined integer (i32 or i64)"),
        }
    };
...
fn inject_start(
    ...
    let aty = addr_ty(memory64);
    let local = module.locals.add(aty);
    let thread_count = module.locals.add(ValType::I32);
    let stack_size = module.locals.add(aty);

    let malloc = find_function(module, "__wbindgen_malloc")?;

    let prev_start = wasm_conventions::get_start(module);
    let mut builder = FunctionBuilder::new(&mut module.types, &[aty], &[]);
    ...
    //
    // The thread counter is always an i32 value stored in memory.
    // On memory64 the address is i64, but the atomic op is still i32.
    push_addr(&mut body, thread_counter_addr, memory64);
    body.i32_const(1)
    ...
    push_usize(&mut body, tls.size, memory64);
    push_usize(&mut body, tls.align, memory64);
    body.call(malloc)
        .global_set(tls.base)
        .global_get(tls.base)
        .call(tls.init);
...
fn inject_destroy(
    ...
    let aty = addr_ty(memory64);
    let mut builder = FunctionBuilder::new(&mut module.types, &[aty, aty, aty], &[]);
    ...
    let tls_base = module.locals.add(aty);
    let stack_alloc = module.locals.add(aty);
    let stack_size = module.locals.add(aty);
    ...
    addr_to_bool(&mut body, memory64);
    body.if_else(
        None,
        |body| {
            body.local_get(tls_base);
            push_usize(body, tls.size, memory64);
            push_usize(body, tls.align, memory64);
            body.call(free);
        },
        |body| {
    ...
    // free the stack calling `__wbindgen_free(stack.alloc, stack.size, align)`
    ...
    body.if_else(
        ...
        |body| {
            ...
            body.local_get(stack_alloc).local_get(stack_size);
            push_usize(body, DEFAULT_THREAD_STACK_SIZE as u64, memory64);
            body.local_get(stack_size);
            addr_to_bool(body, memory64); // select condition must be i32
            body.select(None);
            push_usize(body, 16, memory64);
            body.call(free);
        },
...
fn with_temp_stack(
    ...
    push_addr(body, stack.temp, memory64);
    body.global_set(stack.pointer);

    body.loop_(None, |loop_| {
        let loop_id = loop_.id();

        push_addr(loop_, stack.temp_lock, memory64);
        loop_
            .i32_const(0)
            .i32_const(1)
            ...
            .if_else(
                None,
                |body| {
                    push_addr(body, stack.temp_lock, memory64);
                    body.i32_const(1)
                        .i64_const(-1)
