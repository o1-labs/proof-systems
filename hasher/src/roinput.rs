...
mod tests {
    ...
    fn append_bool() {
        ...
        assert_eq!(roi.bits.len(), 1);
        assert_eq!(roi.bits.as_raw_slice(), [0x01]);
    }

    #[test]
    fn append_two_bits() {
        let roi = ROInput::new().append_bool(false).append_bool(true);
        assert_eq!(roi.bits.len(), 2);
        assert_eq!(roi.bits.as_raw_slice(), [0x02]);
    }
    ...
    fn append_five_bits() {
        let roi = ROInput::new()
            ...
            .append_bool(false)
            .append_bool(true);
        assert_eq!(roi.bits.len(), 5);
        assert_eq!(roi.bits.as_raw_slice(), [0x12]);
    }

    #[test]
    fn append_byte() {
        let roi = ROInput::new().append_bytes(&[0x01]);
        assert_eq!(roi.bits.len(), 8);
        assert_eq!(roi.bits.as_raw_slice(), [0x01]);
    }
    ...
    fn append_two_bytes() {
        ...
        assert_eq!(roi.bits.len(), 16);
        assert_eq!(roi.bits.as_raw_slice(), [0x10, 0xac]);
    }

    #[test]
    fn append_five_bytes() {
        let roi = ROInput::new().append_bytes(&[0x10, 0xac, 0x01, 0xeb, 0xca]);
        assert_eq!(roi.bits.len(), 40);
        assert_eq!(roi.bits.as_raw_slice(), [0x10, 0xac, 0x01, 0xeb, 0xca]);
    }
    ...
    fn append_scalar_and_byte() {
        ...
        assert_eq!(roi.bits.len(), 263);
        assert_eq!(
            roi.bits.as_raw_slice(),
            [
                0x18, 0xb7, 0xef, 0x42, 0x01, 0x28, 0xe6, 0x96, 0x23, 0xc0, 0xc0, 0xdc, 0xfa, 0x28,
                0xd4, 0x7a, 0x02, 0x9d, 0x46, 0x27, 0x20, 0xde, 0xb7, 0x69, 0xd7, 0xb5, 0xdd, 0x6f,
                0x17, 0x44, 0x42, 0x96, 0x00
            ]
        );
    }
    ...
    fn append_two_scalars() {
        ...
        assert_eq!(roi.bits.len(), 510);
        assert_eq!(
            roi.bits.as_raw_slice(),
            [
                0x18, 0xb7, 0xef, 0x42, 0x01, 0x28, 0xe6, 0x96, 0x23, 0xc0, 0xc0, 0xdc, 0xfa, 0x28,
                0xd4, 0x7a, 0x02, 0x9d, 0x46, 0x27, 0x20, 0xde, 0xb7, 0x69, 0xd7, 0xb5, 0xdd, 0x6f,
                0x17, 0x44, 0x42, 0x96, 0xd0, 0xd8, 0x74, 0xa4, 0xc1, 0xad, 0xf1, 0xa0, 0x93, 0x3a,
                0xa4, 0x09, 0x27, 0x87, 0xff, 0xd5, 0xed, 0xe5, 0xca, 0x5b, 0x21, 0xf4, 0xe2, 0xf4,
    ...
    fn append_two_scalars_and_byte() {
        ...
        assert_eq!(roi.bits.len(), 518);
        assert_eq!(
            roi.bits.as_raw_slice(),
            [
                0x60, 0xdb, 0x6f, 0x4f, 0x5b, 0x8c, 0xe1, 0xc7, 0xcb, 0x74, 0x7f, 0xba, 0x9e, 0x32,
                0x4c, 0xc3, 0x26, 0x8c, 0x7a, 0x6e, 0x3f, 0x43, 0xcd, 0x82, 0xd4, 0x51, 0xae, 0x99,
                0xa7, 0xb2, 0xbd, 0x1f, 0x15, 0xff, 0xbb, 0xba, 0x58, 0x03, 0xde, 0x75, 0xac, 0x79,
                0x71, 0x1f, 0x2d, 0xa7, 0xdc, 0x4f, 0xa0, 0x25, 0x47, 0xec, 0x67, 0x15, 0xff, 0x77,
                0xce, 0x4e, 0x0c, 0x80, 0xf8, 0x10, 0x9c, 0xe6, 0x03
            ]
        );
    }
    ...
    fn append_u32() {
        ...
        assert_eq!(roi.bits.len(), 32);
        assert_eq!(roi.bits.as_raw_slice(), [0xc0, 0x07, 0x00, 0x00]);
    }
    ...
    fn append_two_u32_and_bit() {
        let roi = ROInput::new()
            ...
            .append_bool(false)
            .append_u32(u32::MAX);
        assert_eq!(roi.bits.len(), 65);
        assert_eq!(
            roi.bits.as_raw_slice(),
            [0xc1, 0x06, 0x00, 0x00, 0xfe, 0xff, 0xff, 0xff, 0x01]
        );
    }
    ...
    fn append_two_u64_and_bits() {
        ...
        assert_eq!(roi.bits.len(), 130);
        assert_eq!(
            roi.bits.as_raw_slice(),
            [
                0xe1, 0x29, 0x89, 0xd6, 0xcb, 0x3a, 0x15, 0x00, 0x08, 0x17, 0xc4, 0x9b, 0x04, 0xf4,
                0xeb, 0x00, 0x00
            ]
        );
    }
    ...
    fn all_1() {
        ...
        assert_eq!(roi.bits.len(), 641);
        assert_eq!(
            roi.bits.as_raw_slice(),
            [
                0x03, 0xa2, 0xeb, 0xba, 0x64, 0x39, 0x18, 0xa5, 0x53, 0x9f, 0xb5, 0x68, 0x6c, 0x2e,
                0x1a, 0x47, 0xaf, 0x1f, 0x90, 0x98, 0xa8, 0x69, 0x91, 0x07, 0x97, 0xf2, 0x32, 0x43,
                0xce, 0xaf, 0x43, 0x05, 0xc5, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xba, 0xdc,
                0x0f, 0xfe, 0xe7, 0x01, 0x87, 0xe9, 0xb1, 0x25, 0x52, 0x44, 0x89, 0xd0, 0x43, 0x3d,
                0xa7, 0x6f, 0xd8, 0x28, 0x7f, 0xa6, 0x52, 0xea, 0xeb, 0xde, 0x14, 0x7b, 0x45, 0xfa,
                0x0c, 0xd8, 0x6f, 0x17, 0x18, 0x10, 0xff, 0xff, 0xff, 0x7f, 0x01
            ]
        );
    }
