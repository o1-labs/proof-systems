...
//! and $X_{2}$ to $3$.
//! From there, we note `X_{1} X_{2}` as the value `6`, `X_{1}^2` as `4`,
//! `X_{2}^2` as 9. The constant is `1`.
//!
...
//! ```
//!
//! and the polynomial `42 X_{1} + 3 X_{1} X_{2} + 14 X_{2}^2` will be
//! represented as
//!
//! ```text
//! [0,  42,   1,   0,    0,   3,    0,    0,    14]
...
//! ```
//!
//! Adding two polynomials in this base is pretty straightforward: we simply add
//! the coefficients of the two lists.
//!
//! Multiplication is not more complicated.
//! To compute the result of $P_{1} * P_{2}$, the value of index $i$ will be the
//! sum of the decompositions.
//!
...
//! ```
//!
//! To compute `P_{3}`, we get iterate over an empty list of $9$ elements which
//! will define `P_{3}`.
//!
//! For index `1`, we multiply `P_{1}[1]` and `P_{1}[1]`.
//!
//! FOr index $2$, the only way to get this index is by fetching $2$ in each
//! list. Therefore, we do
//! `P_{1}[2] P_{2}[1] + P_{2}[2] * P_{1}[1] = 2 * 3 + 0 * 0 = 6`.
//!
//! For index `3`, same than for `2`.
//!
//! For index `4`, we have `4 = 2 * 2`, therefore, we multiply `P_{1}[2]` and
//! `P_{2}[2]`
//!
//! For index `6`, we have `6 = 2 * 3` and `6 = 3 * 2`, which are the prime
//! decompositions of $6$. Therefore we sum `P_{1}[2] * P_{2}[3]` and
//! `P_{2}[2] * P_{1}[3]`.
//!
...
/// Represents a multivariate polynomial of degree less than `D` in `N`
/// variables. The representation is dense, i.e., all coefficients are stored.
