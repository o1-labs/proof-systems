...
// Prover/Verifier test includidng the Keccak lookups
#[test]
fn test_keccak_prover_lookups() {
    // guaranteed to have at least 30MB of stack
    stacker::grow(30 * 1024 * 1024, || {
        let mut rng = o1_utils::tests::make_test_rng();
        let domain_size = 1 << 8;
        ...
        while keccak_env.constraints_env.step.is_some() {
            ...
            // Run the interpreter, which sets the witness columns
            keccak_env.step();
            // Update the multiplicities of the fixed lookups in this step
            keccak_env.witness_env.lookups();
        ...
        // Obtain updated multiplicities for the whole execution
        let multiplicities = keccak_env.witness_env.multiplicities.clone();
