...
impl<Fp: Field> Env<Fp> {
    pub fn create(_page_size: usize, state: State, preimage_oracle: PreImageOracle) -> Self {
        ...
        let max_index = (&state
            ...
            .max()
            .unwrap();
        let max_memory_address = max_index * 4096;
        // Allocating 4 times the initial memory.
        let initially_allocate_memory_address_space: usize = (max_memory_address * 4) as usize;
        let mut initial_memory: Vec<u8> = vec![0; initially_allocate_memory_address_space];
        ...
        let memory_write_index = Vec::with_capacity(initially_allocate_memory_address_space);
        ...
        if self.should_trigger_at(at) {
            ...
            let memory = self
                ...
                .map(|(i, _d)| {
                    ...
                    Page { index: i as u32, data: d.to_vec() }
                })
                .collect();
            let s: State = State {
