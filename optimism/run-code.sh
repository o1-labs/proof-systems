...
export L1_HEAD=$L1_FINALIZED_HASH
export L2_CLAIM=$OUTPUT_ROOT
export L2_BLOCK_NUMBER=$OUTPUT_L2BLOCK_NUMBER
...
export STARTING_OUTPUT_ROOT=$(echo ${OUTPUT} | cut -d' ' -f 1)
L2_HEAD_NUMBER=$(echo ${OUTPUT} | cut -d' ' -f 3)
export L2_HEAD=$(cast block "${L2_HEAD_NUMBER}" --rpc-url "${L2RPC}" -f hash)

TODAY=$(date +"%Y-%m-%d-%H-%M-%S")
FILENAME=${TODAY}-op-program-data-log.sh
export OP_PROGRAM_DATA_DIR=$(pwd)/op-program-db-sepolia-${TODAY}
