...
fn poseidon_tweedledee()
{
    ...
    for _ in 0..NUM_POS-1
    {
        ...
        {
            gates.push(CircuitGate::<Fp>::create_poseidon(GateWires::wires((i, (i+PERIOD)%M), (i+N, N+((i+PERIOD)%M)), (i+2*N, 2*N+((i+PERIOD)%M))), [c[j][0],c[j][1],c[j][2]]));
            i+=1;
        }
    ...
    {
        gates.push(CircuitGate::<Fp>::create_poseidon(GateWires::wires((i, i), (i+N, N+(i)), (i+2*N, 2*N+(i))), [c[j][0],c[j][1],c[j][2]]));
        i+=1;
    }
