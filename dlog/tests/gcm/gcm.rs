...
impl Gcm
{
    ...
    pub fn encrypt (&mut self, aad: &Vec<u8>, pt: &Vec<u8>) -> (Vec<u8>, [u8; 16])
    {
        ...
        let x = ht.iter().zip(sz.to_be_bytes().iter()).map(|(h, e)| super::aes::xor2(*h, *e)).collect::<Vec<_>>();
        ht = mul(self.h, x[0..16].try_into().unwrap());
...
pub fn mul_helper(xb: [u8; 16], yb: [u8; 16]) -> [u8; 16]
{
    let x: u128 = u128::from_be_bytes(xb);
    let y: u128 = u128::from_be_bytes(yb);
