...
use oracle::{
    poseidon::{ArithmeticSponge, PlonkSpongeConstants5W, Sponge, SpongeConstants},
    sponge::{DefaultFqSponge, DefaultFrSponge},
};
use plonk_5_wires_circuits::wires::Wire;
use plonk_5_wires_circuits::{constraints::ConstraintSystem, gate::CircuitGate};
...
fn positive(index: &Index<Affine>) {
    ...
    let mut sponge = ArithmeticSponge::<Fq, PlonkSpongeConstants5W>::new();
    ...
    println!(
        "{}{:?}",
        "Number of Poseidon hashes in the circuit: ".yellow(),
        NUM_POS
    );
    println!(
        "{}{:?}",
        "Full rounds: ".yellow(),
        PlonkSpongeConstants5W::ROUNDS_FULL
    );
    println!(
        "{}{:?}",
        "Sbox alpha: ".yellow(),
        PlonkSpongeConstants5W::SPONGE_BOX
    );
    ...
    for test in 0..1 {
        ...
        w.iter_mut().for_each(|x| {
            x.extend(vec![Fq::rand(rng); padding_count]);
        });
        ...
        batch.push(
            ProverProof::create::<
                DefaultFqSponge<TweedledumParameters, PlonkSpongeConstants5W>,
                DefaultFrSponge<Fq, PlonkSpongeConstants5W>,
