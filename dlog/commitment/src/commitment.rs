...
use algebra::{BitIterator, LegendreSymbol, UniformRand, Field, SquareRootField, PrimeField, AffineCurve, ProjectiveCurve, VariableBaseMSM};
...
fn b_poly<F : Field>(chals : &Vec<F>, chal_invs : &Vec<F>, x : F) -> F {
    ...
    product((0..k).map(|i| {
        (chal_invs[i] + &(chals[i] * &pow_twos[k - 1 - i]))
    }))
}
...
fn pows<F : Field>(d : usize, x : F) -> Vec<F> {
    let mut acc = F::one();
    (0..d).map(|_| { let r = acc; acc *= &x; r }).collect()
}

fn squeeze_square_challenge<Fq : Field, G, Fr: SquareRootField, EFqSponge: FqSponge<Fq, G, Fr>>(sponge : &mut EFqSponge) -> Fr {
...
fn squeeze_sqrt_challenge<Fq : Field, G, Fr: SquareRootField, EFqSponge: FqSponge<Fq, G, Fr>>(sponge : &mut EFqSponge) -> Fr {
...
impl<G: AffineCurve> SRS<G>
{
    ...
    ) -> Result<OpeningProof<G>, ProofError>
    {
        ...
        // TODO: Do something better than padding
        ...
        for _ in 0..rounds {
            ...
            let l = {
                ...
                (VariableBaseMSM::multi_scalar_mul(&g_hi, &a_lo )
                    + &(self.h.mul(rand_l) + &u.mul(a_lo_b_hi))).into_affine()
            };
            ...
            let r  = {
                ...
                (VariableBaseMSM::multi_scalar_mul(&g_lo, &a_hi )
                    + &(self.h.mul(rand_r) + &u.mul(a_hi_b_lo))).into_affine()
            };
            ...
            a = a_hi.iter().zip(a_lo).map(|(&hi, &lo)| {
                let mut res = hi * &u_inv;
                res += &(lo * &u);
                res
            }).collect();

            b = b_lo.iter().zip(b_hi).map(|(&lo, &hi)| {
            ...
            g = {
                let mut g_proj : Vec<G::Projective> = {
                    ...
                    pairs.into_par_iter().map(|(lo, hi)| {
                        shamir_sum::<G>(u_inv, *lo, u, hi)
                    }).collect()
                };
        ...
        let r_prime = blinders.iter().zip(chals.iter().zip(chal_invs.iter())).map(
            | ((l, r), (u, u_inv)) | {
            ((*l) * & u.square()) + &(*r * & u_inv.square())
        }).fold(blinding_factor, |acc, x| acc + &x);

        let d = Fr::<G>::rand(rng);
        let r_delta = Fr::<G>::rand(rng);

        let delta = ((g0.into_projective() + &(u.mul(b0))).into_affine().mul(d) + & self.h.mul(r_delta)).into_affine();
    ...
    ) -> bool
    {
        ...
        for (evaluation_points, xi, r, polys, opening) in batch.iter()
        {
            ...
            let chal_squareds : Vec<_> = opening.lr.iter().map(|(l, r)| {
                    sponge.absorb_g(l);
                    sponge.absorb_g(r);
            ...
            // - rand_base_i * (z1 * < s, self.g > )
            {
                let terms : Vec<_> = s.par_iter().map(|s| {
            ...
            for ((l, r), (u, u_inv)) in opening.lr.iter().zip(chal_squareds.iter().zip(chal_squared_invs.iter())) {
                points.push(*l);
                ...
                for (comm, evals, shifted) in polys {
                    ...
                    match shifted {
                        Some((shifted_comm_i, m)) => {
                            ...
                            let shifted_evals : Vec<_> = evaluation_points.iter().zip(evals).map(|(elm, f_elm)| {
                                elm.pow(& [(self.g.len() - m) as u64]) * f_elm
                            }).collect();
