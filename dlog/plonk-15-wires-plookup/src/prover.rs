    ...
    pub fn create<EFqSponge: Clone + FqSponge<Fq<G>, G, Fr<G>>, EFrSponge: FrSponge<Fr<G>>>(
        ...
        let mut lkppolys = LookupPolys::<Fr<G>> {
            ...
            lw: &lkpevl.lw.interpolate_by_ref()
                + &DensePolynomial::rand(1, rng).mul_by_vanishing_poly(index.pcs.cs.domain.d1),
        ...
        let (mut t, res) = (&(&t4.interpolate() + &t8.interpolate()) + &(&genp + &posp))
            .divide_by_vanishing_poly(index.pcs.cs.domain.d1)
        ...
        let evlp = [
            oracles.po.zeta,
            oracles.po.zeta * &index.pcs.cs.domain.d1.group_gen,
        ];
        let evals = evlp
            .iter()
            .map(|e| PPE::<Vec<Fr<G>>> {
                pe: PE::<Vec<Fr<G>>> {
                    s: array_init(|i| {
                        index.pcs.cs.sigmam[0..COLUMNS - 1][i].eval(*e, index.max_poly_size)
                    }),
        ...
        let f = &(&(&(&(&(&(&index.pcs.cs.gnrc_lnrz(&pe[0].w)
            ...
            + &index
                .pcs
                .cs
                .perm_lnrz(&pe, &oracles.po, &alpha[range::PERM]);
        ...
        polynomials.extend(
            index.pcs.cs.sigmam[0..COLUMNS - 1]
                .iter()
        ...
        polynomials.extend(vec![(&index.pcs.tablem, None, non_hiding(1))]);
