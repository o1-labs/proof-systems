...
where
    G::ScalarField: CommitmentField,
{
    // plonk-plookup constraints system polynomials
    ...
    G::ScalarField: CommitmentField,
{
    pub fn verifier_index(&self) -> VerifierIndex<G> {
        ...
        VerifierIndex {
            ...
            sigma_comm: array_init(|i| {
                srs.get_ref()
                    .commit_non_hiding(&self.pcs.cs.sigmam[i], None)
            }),
            ...
            rcm_comm: array_init(|i| {
                array_init(|j| {
                    srs.get_ref()
                        .commit_non_hiding(&self.pcs.cs.rcm[i][j], None)
                })
            }),
    ...
    pub fn create(
        ...
        if pcs.cs.public > 0 {
            assert!(
                max_poly_size >= pcs.cs.domain.d1.size as usize,
