...
impl<G: CommitmentCurve> ProverProof<G> where G::ScalarField : CommitmentField
{
    ...
    ) -> (EFqSponge, Fr<G>, RandomOracles<Fr<G>>, Vec<Fr<G>>, [Vec<Fr<G>>; 2], [Fr<G>; 2], Vec<(PolyComm<G>, Vec<Vec<Fr<G>>>)>, Fr<G>, Fr<G>)
    {
        ...
        let combined_inner_product = {
            ...
            es.extend((0..COLUMNS).map(|c| (self.evals.iter().map(|e| &e.pe.w[c]).collect::<Vec<_>>(), None)).collect::<Vec<_>>());
            es.extend(
                vec!
                [
    ...
    ) -> Result<bool, ProofError>
    {
        ...
        (
            |(index, lgr_comm, proof)|
            {
                ...
                s.push(CS::endomul_scalars(&pevals, index.endo, &alpha[range::ENDML]));
                p.push(&index.emul_comm);

                // lookup
                s.push(ConstraintSystem::lookup_scalars(&pevals, &alpha[range::LKP]));
        ...
        (
            |((index, _lgr_comm, proof), (p_eval, p_comm, f_comm, fq_sponge, oracles, polys))|
            {
                ...
                polynoms.extend(proof.commitments.w_comm.iter().zip((0..COLUMNS).map(|i| proof.evals.iter().map(|e| &e.pe.w[i]).
                    collect::<Vec<_>>()).collect::<Vec<_>>().iter()).map(|(c, e)| (c, e.clone(), None)).collect::<Vec<_>>());
                polynoms.extend
                (
                    vec!
                    [
                        (&proof.commitments.z_comm, proof.evals.iter().map(|e| &e.pe.z).collect::<Vec<_>>(), None),
