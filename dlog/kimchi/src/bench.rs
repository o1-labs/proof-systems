...
impl BenchmarkCtx {
    ...
    pub fn create_proof(&self) -> ProverProof<Affine> {
        ...
        // create witness
        let mut witness: [Vec<Fp>; COLUMNS] = array_init(|_| vec![Fp::zero(); CIRCUIT_SIZE]);
        for row in 0..self.num_gates {
            witness[0][row] = 3u32.into();
            witness[1][row] = 5u32.into();
            witness[2][row] = Fp::from(3u32 * 5);
        }
...
mod tests {
    ...
    #[test]
    fn test_long_bench() {
        // context created in 23.3520 ms
        let start = Instant::now();
        let ctx = BenchmarkCtx::new(1 << 15);
        println!("context created in {}", start.elapsed().as_millis());

        // proof created in 8.1972 ms
        let start = Instant::now();
        let proof = ctx.create_proof();
        println!("proof created in {}", start.elapsed().as_millis());

        // proof verified in 1.802 ms
        let start = Instant::now();
        ctx.batch_verification(vec![proof.clone()]);
        println!("proof verified in {}", start.elapsed().as_millis());
    }
