...
use ark_poly::{Evaluations, Radix2EvaluationDomain as R2D, Radix2EvaluationDomain};
use folding::{
...
pub fn test_simple_add() {
    ...
    let domain_size: usize = 1 << 16; // Has to be 1 << 15, but then we don't fit
    ...
    //pub struct Config<const N_COL: usize, const N_FSEL: usize>;
    ...
    const N_ALPHAS_INIT: usize = 489; // number of constraints we have before quad
    ...
    const N_COL_QUAD: usize = 345;
    const N_COL_TOTAL_QUAD: usize = N_COL_TOTAL + N_COL_QUAD;

    println!("Generating fixed selectors");
    ...
    assert!(
        ivc_fixed_selectors.len() == N_FSEL_TOTAL,
        "Real number of fixed selectors ({:?}) is not matching expected {N_FSEL_TOTAL}",
        ivc_fixed_selectors.len()
    );
    ...
    type Config<const N_COL: usize, const N_FSEL: usize> = StandardConfig<
        ...
        Column,
        Challenge,
        PlonkishInstance<N_COL>,
        PlonkishWitness<N_COL, N_FSEL>,
        (),
        GenericVecStructure<BN254G1Affine>,
    >;
    ...
    let ivc_constraints: Vec<E<Fp>> = {
        let mut ivc_constraint_env = ConstraintBuilderEnv::<Fp, LT>::create();
    ...
    let ivc_compat_constraints_mapped: Vec<FoldingCompatibleExpr<MainTestConfig>> =
    ...
    let (_folding_scheme, _real_folding_compat_constraints) = FoldingScheme::<MainTestConfig>::new(
        folding_compat_constraints.clone(),
        &srs,
        domain.d1,
        &structure,
    ...
    let folding_witness_one = PlonkishWitness {
        witness: joint_witness_one
            ...
            .collect(),
        fixed_selectors: ivc_fixed_selectors_evals.clone().try_into().unwrap(),
    };
    ...
    assert_eq!(
        ...
        "Expected {N_COL_QUAD} columns, got {additional_columns}"
    );
