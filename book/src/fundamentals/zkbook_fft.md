...
2.  Multiply $p$ and $q$ in evaluation form by multiplying their points pairwise in $O(n)$
3.  Convert $p$ and $q$ back to to coefficient form in $O(n\log n)$ using the inverse Cooley-Tukey FFT
