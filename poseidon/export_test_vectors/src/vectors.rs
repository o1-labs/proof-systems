...
mod tests {
    ...
    // This function normalizes the version information in the ES5 header to
    // ensure that tests don't fail due to differences in the version string
    ...
    fn test_export_regression_all_formats() {
        ...
        for (mode, param_type, format, expected_file) in test_cases {
            ...
            match format {
                ...
                OutputFormat::Es5 => {
                    assert_eq!(
                        normalize_es5_header_version(generated_content.trim()),
                        normalize_es5_header_version(expected_content.trim()),
                        "Generated ES5 output doesn't match expected file (ignoring version header): {}",
                        expected_file
                    );
                }
            }
