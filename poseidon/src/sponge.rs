    ...
    <P::BaseField as PrimeField>::BigInt: Into<<P::ScalarField as PrimeField>::BigInt>,
{
    ...
    fn digest(mut self) -> P::ScalarField {
        ...
        // This means that there is a bias for the value zero (in one of the
        // curve).
        // An attacker could try to target that seed, in order to predict the
        // challenges u and v produced by the Fr-Sponge.
        // This would allow the attacker to mess with the result of the
        // aggregated evaluation proof.
        // Previously the attacker's odds were 1/q, now it's (q-p)/q.
        // Since log2(q-p) ~ 86 and log2(q) ~ 254 the odds of a successful
        // attack are negligible.
