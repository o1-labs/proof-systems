use crate::pickles::multiplicities_columns::*;
    ...
    G::BaseField: PrimeField,
{
    ...
    let zeta_n = zeta.pow([domain.d1.size]);
    let (quotient_zeta, _) = evaluations.zeta.quotient_chunks.iter().fold(
        ...
        |(res, zeta_i_n), chunk| {
            ...
            let zeta_i_n = zeta_i_n * zeta_n;
            (res, zeta_i_n)
        },
    );

    let quotient_is_correct = quotient_zeta == numerator_zeta / ((zeta_n) - G::ScalarField::one());
