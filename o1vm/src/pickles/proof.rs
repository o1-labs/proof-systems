...
impl<G: KimchiCurve> ProofInputs<G> {
    ...
    pub fn witness17(&self) {
        ...
        for i in 0..self.evaluations.selector.len() {
            if !(G::ScalarField::from(1 as u64) == self.evaluations.selector[i]) {
                assert!(G::ScalarField::from(0 as u64) == self.evaluations.selector[i]);
            }
            if G::ScalarField::from(1 as u64) == self.evaluations.selector[i] && count < 5 {
                count += 1;
                for j in &l {
