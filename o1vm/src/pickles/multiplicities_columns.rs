...
impl<X> IntoIterator for ColumnEnv<X> {
    ...
    type IntoIter = Chain<
        Chain<
            Chain<
                <FixedLookup<X> as IntoIterator>::IntoIter,
                <FixedLookup<X> as IntoIterator>::IntoIter,
            >,
            <FixedLookup<X> as IntoIterator>::IntoIter,
        >,
...
impl<X> ColumnEnv<X> {
        ...
        Self: Sized,
    {
        ...
        ColumnEnv {
            fixedlookup: fixedlookup.map(&mut f),
...
pub struct LookupProofInput<F: PrimeField> {
    pub fixedlookup: FixedLookup<Vec<Vec<F>>>,
    pub multiplicities: FixedLookup<Vec<F>>,
...
impl<F: Field> Index<MultiplicitiesChallengeTerm> for MultiplicitiesChallenges<F> {
    ...
    fn index(&self, term: MultiplicitiesChallengeTerm) -> &Self::Output {
        let MultiplicitiesChallenges { beta, gamma } = self;
        match term {
...
impl<'a> AlphaChallengeTerm<'a> for MultiplicitiesChallengeTerm {
    const ALPHA: Self = panic!("alpha doesnt exists");
}
    ...
    for MultiplicitiesEvalEnvironment<'a, F>
{
    ...
    fn get_column(&self, col: &Self::Column) -> Option<&'a Evaluations<F, D<F>>> {
        ...
        match col {
            ...
            FixedLookup(id) => Some(&fixedlookup[*id]),
...
impl<F> ColumnEnv<F> {
    pub fn get_column(&self, col: &MultiplicitiesColumns) -> Option<&F> {
        ...
        match *col {
            MultiplicitiesColumns::FixedLookup(id) => Some(&fixedlookup[id]),
