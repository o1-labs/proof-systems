...
pub fn prove(
    ...
    let base_sponge_before_evaluations = base_sponge.clone();
    let mut scalar_sponge = CurveScalarSponge::new(Curve::sponge_params());
    scalar_sponge.absorb(&base_sponge.digest());
    ...
    let opening_proof =
        srs.open(
            ...
            ScalarField::one(), // Single evaluation, so we don't care
            base_sponge_before_evaluations,
            rng,
        );
...
pub fn verify_wrt_combined_data_commitment(
    ...
    srs.verify(
        ...
        &mut [BatchEvaluationProof {
            sponge: base_sponge_before_evaluations,
