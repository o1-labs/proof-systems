...
/// Copies in `buffer` the `Fp::size_in_bytes()` representation of `x`
/// (for Pallas & Vesta, the representation is 32 bytes)
pub fn decode_into_full<Fp: PrimeField>(buffer: &mut [u8], x: Fp) {
    let bytes = x.into_bigint().to_bytes_be();
    buffer.copy_from_slice(&bytes);
}

/// Returns the `Fp::size_in_bytes()` representation of `x`
/// (for Pallas & Vesta, the representation is 32 bytes)
