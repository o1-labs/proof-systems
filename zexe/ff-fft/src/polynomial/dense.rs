...
mod tests {
    ...
    fn double_polynomials_random() {
        ...
        for degree in 0..70 {
            let p = DensePolynomial::<Fp>::rand(degree, rng);
            let p_double = &p + &p;
    ...
    fn add_polynomials() {
        ...
        for a_degree in 0..70 {
            for b_degree in 0..70 {
                let p1 = DensePolynomial::<Fp>::rand(a_degree, rng);
                let p2 = DensePolynomial::<Fp>::rand(b_degree, rng);
                let res1 = &p1 + &p2;
                let res2 = &p2 + &p1;
                assert_eq!(res1, res2);
            }
    ...
    fn sub_polynomials() {
        let rng = &mut test_rng();
        let p1 = DensePolynomial::<Fp>::rand(5, rng);
        let p2 = DensePolynomial::<Fp>::rand(3, rng);
        let res1 = &p1 - &p2;
        let res2 = &p2 - &p1;
        assert_eq!(
            &res1 + &p2,
    ...
    fn divide_polynomials_fixed() {
        ...
        let divisor = DensePolynomial::from_coefficients_slice(&[Fp::one(), Fp::one()]); // Construct a monic linear polynomial.
    ...
    fn divide_polynomials_random() {
        ...
        for a_degree in 0..70 {
            for b_degree in 0..70 {
                let dividend = DensePolynomial::<Fp>::rand(a_degree, rng);
                let divisor = DensePolynomial::<Fp>::rand(b_degree, rng);
    ...
    fn mul_polynomials_random() {
        ...
        for a_degree in 0..70 {
            for b_degree in 0..70 {
                let a = DensePolynomial::<Fp>::rand(a_degree, rng);
                let b = DensePolynomial::<Fp>::rand(b_degree, rng);
    ...
    fn test_leading_zero() {
        ...
        let leading_coefficient: Fp = coefficients[n];
