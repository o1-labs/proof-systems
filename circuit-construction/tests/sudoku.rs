    ...
    F: Field,
{
    ...
    pub fn solve(sudoku: &Sudoku<F>) -> Self {
        let from = |a: i32| {
            if a > 0 {
                F::from(a as u32)
            } else {
    ...
    F: PrimeField,
{
    ...
    fn new<Sys>(
        ...
        witness: Option<&SudokuSolution<F>>,
    ) -> Self
    where
        Sys: Cs<F>,
    {
    ...
    fn from_witness<Sys>(sys: &mut Sys, solution: Option<&SudokuSolution<F>>) -> Self
    where
        Sys: Cs<F>,
    {
        let mut sudoku = vec![];
