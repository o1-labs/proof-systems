...
use oracle::{rndoracle::ProofError, sponge::FqSponge, utils::PolyUtils};
...
impl<E: PairingEngine> ProverProof<E> {
    ...
    pub fn create<EFqSponge: FqSponge<E::Fq, E::G1Affine, E::Fr>, EFrSponge: FrSponge<E::Fr>>(
        ...
        let p = -Evaluations::<E::Fr, D<E::Fr>>::from_vec_and_domain(
            public.clone(),
            index.cs.domain.d1,
        )
        .interpolate();
        ...
        let l = &Evaluations::<E::Fr, D<E::Fr>>::from_vec_and_domain(
            index
                .cs
                .gates
                .iter()
        ...
        let r = &Evaluations::<E::Fr, D<E::Fr>>::from_vec_and_domain(
            index
                .cs
                .gates
                .iter()
        ...
        .interpolate()
            + &DensePolynomial::rand(1, &mut OsRng).mul_by_vanishing_poly(index.cs.domain.d1);
        let o = &Evaluations::<E::Fr, D<E::Fr>>::from_vec_and_domain(
            index
                .cs
                .gates
                .iter()
        ...
        z.iter_mut().skip(1).enumerate().for_each(|(j, x)| {
            *x = (witness[j] + &(index.cs.sigmal1[0][j] * &oracles.beta) + &oracles.gamma)
        ...
        if z.pop().unwrap() != E::Fr::one() {
            return Err(ProofError::ProofCreation);
        };
        let z = Evaluations::<E::Fr, D<E::Fr>>::from_vec_and_domain(z, index.cs.domain.d1)
            .interpolate();
        ...
        let (bnd, res) = DenseOrSparsePolynomial::divide_with_q_and_r(
            &(&z - &DensePolynomial::from_coefficients_slice(&[E::Fr::one()])).into(),
        ...
        let f1 = &(&(&(&index.cs.qmm.scale(evals.l * &evals.r) + &index.cs.qlm.scale(evals.l))
        ...
        let f3 = index.cs.sigmam[2].scale(
        ...
        Ok(Self {
            ...
            proof2: index.urs.get_ref().open(
                vec![&z],
                oracles.v,
                oracles.zeta * &index.cs.domain.d1.group_gen,
            )?,
            evals,
            public,
        })
