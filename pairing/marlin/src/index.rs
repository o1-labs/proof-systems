...
use algebra::{
    curves::models::short_weierstrass_jacobian::GroupAffine as SWJAffine, AffineCurve,
    PairingEngine,
};
use commitment_pairing::urs::URS;
use ff_fft::EvaluationDomain;
use marlin_circuits::{domains::EvaluationDomains, gate::CircuitGate};
...
impl<'a, E: PairingEngine> URSValue<'a, E> {
    pub fn generate<'b>(ds: EvaluationDomains<E::Fr>, rng: &'b mut dyn RngCore) -> URS<E> {
        let max_degree = *[3 * ds.h.size() - 1, ds.b.size()].iter().max().unwrap();

        URS::<E>::create(max_degree, vec![ds.h.size() - 1, ds.k.size() - 1], rng)
    }
    ...
    E::G1Affine: CoordinatesCurve,
{
    ...
    pub fn create<'b>(
        ...
        let nonzero_entries: usize = [&a, &b, &c]
            .iter()
            .map(|x| x.nnz())
            .max()
        ...
        let domains =
            EvaluationDomains::create(a.shape().0, a.shape().1, public_inputs, nonzero_entries)
...
impl<'a, E: PairingEngine> Index<'a, E> {
    ...
    pub fn verifier_index(&self) -> VerifierIndex<E> {
        ...
        for i in [self.domains.h.size() - 1, self.domains.k.size() - 1].iter() {
            let i = max_degree - i;
            hn.insert(i, self.urs.get_ref().hn.get(&i).unwrap().clone());
        }

        let urs = {
            let gp = (0..self.domains.x.size())
                .map(|i| self.urs.get_ref().gp[i])
                .collect();
            URS::<E> {
                gp,
                hn,
        ...
        VerifierIndex {
            matrix_commitments: [
                Self::matrix_values(a),
                Self::matrix_values(b),
                Self::matrix_values(c),
            ],
    ...
    pub fn verify(&self, witness: &Vec<E::Fr>) -> bool {
        if self.compiled[0].constraints.shape().1 != witness.len() {
            return false;
        }
        ...
        for gate in gates.iter() {
            if gate.wire[0] * &gate.wire[1] != gate.wire[2] {
                return false;
            }
        }
        true
