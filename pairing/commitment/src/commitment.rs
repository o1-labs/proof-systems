...
use algebra::{
    AffineCurve, Field, One, PairingEngine, PrimeField, ProjectiveCurve, UniformRand,
...
impl<E: PairingEngine> URS<E> {
    ...
    pub fn commit(&self, plnm: &DensePolynomial<E::Fr>) -> Result<E::G1Affine, ProofError> {
        if plnm.coeffs.len() > self.depth {
    ...
    pub fn commit_with_degree_bound(
        ...
        if self.depth < max || plnm.coeffs.len() > max {
            return Err(ProofError::PolyCommitWithBound);
        }
    ...
    pub fn exponentiate_sub_domain(
        ...
        if plnm.coeffs.len() > self.depth {
            return Err(ProofError::PolyExponentiate);
        }

        Ok(VariableBaseMSM::multi_scalar_mul(
            &(0..plnm.len())
                .map(|i| self.gp[ratio * i])
                .collect::<Vec<_>>(),
            &plnm
                .coeffs
                .iter()
    ...
    pub fn verify(
            ...
            for x2 in x1.2.iter() {
                match x2.2 {
                    Some((p, m)) => {
                        ...
                        if !shifted.contains_key(&m) {
                            shifted.insert(m, Vec::new());
                        }
        ...
        for max in shifted.keys() {
            if !self.hn.contains_key(&(self.depth - max)) {
                return false;
            }
            table.push((
        ...
        table.push((
            VariableBaseMSM::multi_scalar_mul(&open_point, &open_scalar)
                .into_affine()
                .into(),
            self.hx.into(),
        ));
        table.push((
            VariableBaseMSM::multi_scalar_mul(&openy_point, &openy_scalar)
                .into_affine()
                .into(),
