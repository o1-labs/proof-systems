...
use algebra::{
    AffineCurve, Field, FixedBaseMSM, FromBytes, One, PairingEngine, PrimeField, ProjectiveCurve,
    ToBytes, UniformRand, VariableBaseMSM,
};
...
impl<E: PairingEngine> URS<E> {
    ...
    pub fn create(depth: usize, degrees: Vec<usize>, rng: &mut dyn RngCore) -> Self {
        ...
        let mut hn = FixedBaseMSM::multi_scalar_mul::<E::G2Projective>(
            ...
            &degrees
                .iter()
                .map(|i| x.pow([(depth - *i) as u64]))
                .collect::<Vec<E::Fr>>(),
        );
        ProjectiveCurve::batch_normalization(&mut hn);
        let mut hnh: HashMap<usize, E::G2Affine> = HashMap::new();
        for (i, p) in degrees.iter().zip(hn.iter()) {
            hnh.insert(depth - *i, p.into_affine());
        }
    ...
    pub fn update(&mut self, rng: &mut dyn RngCore) {
        let mut x = E::Fr::rand(rng);
        let mut cur = E::Fr::one();
    ...
    pub fn check(&mut self, hp1: E::G2Affine, rng: &mut dyn RngCore) -> bool {
        let xy = <E>::pairing(self.prf, hp1);
        // verify hx consistency with zk-proof
        pairing_check!(
            E::G1Projective::prime_subgroup_generator(),
            E::G2Projective::from(self.hx),
            xy
        );
        // verify gp[1] consistency with zk-proof
        pairing_check!(
            E::G1Projective::from(self.gp[1]),
            E::G2Projective::prime_subgroup_generator(),
            xy
        );
        ...
        for x in self.hn.iter() {
            ...
            if <E>::pairing(self.gp[*x.0], *x.1) != fk {
                return false;
            }
        }

        let rand = (1..self.gp.len())
            .map(|_| E::Fr::rand(rng).into_repr())
            .collect::<Vec<_>>();
