...
use crate::{Fp, MsmBN254, MsmBN254G2Affine, DOMAIN_SIZE};
...
pub fn test_serialization() {
    ...
    let srs: PairingSRS<MsmBN254> = PairingSRS::create(x, DOMAIN_SIZE);

    // minimal example: a single G2 point
    let actual: MsmBN254G2Affine = srs.verifier_srs.h;
    ...
    let expected: MsmBN254G2Affine = CanonicalDeserialize::deserialize(reader).unwrap();
...
pub fn create_and_store_srs(
    ...
    let mut srs: PairingSRS<MsmBN254> = PairingSRS::create(x, DOMAIN_SIZE);
