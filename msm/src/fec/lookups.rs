...
impl<Ff: PrimeField> LookupTableID for LookupTable<Ff> {
    ...
    fn from_u32(value: u32) -> Self {
        match value {
            1 => Self::RangeCheck15(PhantomData),
    ...
    fn ix_by_value<F: PrimeField>(&self, value: F) -> usize {
        match self {
            Self::RangeCheck15(_) => TryFrom::try_from(value.to_biguint()).unwrap(),
            Self::RangeCheck4Abs => {
...
impl<Ff: PrimeField> LookupTable<Ff> {
    ...
    pub fn entries<F: PrimeField>(&self, domain_d1_size: u64) -> Vec<F> {
        ...
        match self {
            Self::RangeCheck1Abs => [F::one(), F::zero() - F::one()]
                ...
                .collect(),
            Self::RangeCheck15(_) => (0..domain_d1_size).map(|i| F::from(i)).collect(),
    ...
    pub fn is_member<F: PrimeField>(&self, value: F) -> bool {
        match self {
            ...
            Self::RangeCheck15(_) => value.to_biguint() < BigUint::from(2u128.pow(15)),
