...
pub fn caml_pasta_fp_poseidon_block_cipher(mut cx: FunctionContext) -> JsResult<JsUint8Array> {
    ...
    let mut state: Vec<Fp> = FlatVector::<WasmPastaFp>::from_bytes(state.as_slice(&cx).to_vec())
        .into_iter()
        .map(Into::into)
        .collect();
...
pub fn caml_pasta_fq_poseidon_block_cipher(mut cx: FunctionContext) -> JsResult<JsUint8Array> {
    ...
    let mut state: Vec<Fq> = FlatVector::<WasmPastaFq>::from_bytes(state.as_slice(&cx).to_vec())
        .into_iter()
        .map(Into::into)
        .collect();
