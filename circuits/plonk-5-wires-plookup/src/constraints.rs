...
impl<F: FftField + SquareRootField> ConstraintSystem<F> {
    pub fn create(
        ...
        let domain = EvaluationDomains::<F>::create(if gates.len() > tbl.len() {
            gates.len()
        } else {
            tbl.len()
        })?;
        ...
        .interpolate();
        let lkpm = E::<F, D<F>>::from_vec_and_domain(
            gates.iter().map(|gate| gate.lookup()).collect(),
            domain.d1,
        )
        .interpolate();
        ...
        // constant polynomials
        let l18 = DP::from_coefficients_slice(&[F::zero(), F::one()])
        ...
        Some(ConstraintSystem {
            ...
            ps4: E::<F, D<F>>::from_vec_and_domain(
                (0..domain.d4.size)
                    .map(|j| ps8.evals[2 * j as usize])
                    .collect(),
                domain.d4,
            ),
            ps8,
            ...
            packm,
            lkpl4: E::<F, D<F>>::from_vec_and_domain(
                (0..domain.d4.size)
                    .map(|j| lkpl8.evals[2 * j as usize])
                    .collect(),
                domain.d4,
            ),
            lkpl8,
            ...
            // constant polynomials
            l14: E::<F, D<F>>::from_vec_and_domain(
                (0..domain.d4.size)
                    .map(|j| l18.evals[2 * j as usize])
                    .collect(),
                domain.d4,
            ),
            l18,
    ...
    pub fn evaluate1(&self, w: &[DP<F>; COLUMNS], z: &DP<F>) -> WitnessOverDomains<F> {
        ...
        let z4 = E::<F, D<F>>::from_vec_and_domain(
            (0..self.domain.d4.size)
                .map(|j| z8.evals[2 * j as usize])
                .collect(),
            self.domain.d4,
        );
    ...
    pub fn evaluate2(&self, polys: &LookupPolys<F>) -> LookupShifts<F> {
