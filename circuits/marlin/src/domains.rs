...
impl<F: FftField> EvaluationDomains<F> {
    pub fn create(
        ...
        let h_group_size = {
            let m = if constraints > variables {
                constraints
            } else {
                variables
            };
        ...
        let x_group_size = D::<F>::compute_size_of_domain(public_inputs)?;
        let k_group_size = D::<F>::compute_size_of_domain(nonzero_entries)?;
    ...
    pub fn evals_from_coeffs(v: Vec<F>, d: D<F>) -> Evaluations<F, GeneralEvaluationDomain<F>> {
