...
impl<F: FftField + SquareRootField> ConstraintSystem<F> {
    pub fn create(
        ...
        let mut padding = (gates.len()..n)
            .map(|i| {
                CircuitGate::<F>::zero(GateWires::wires(
                    (i, i),
                    (n + i, n + i),
                    (2 * n + i, 2 * n + i),
                ))
        ...
        let sigmam: [DensePolynomial<F>; 3] = array_init(|i| {
            Evaluations::<F, D<F>>::from_vec_and_domain(sigmal1[i].clone(), domain.d1).interpolate()
        });
        ...
        let qlm = Evaluations::<F, D<F>>::from_vec_and_domain(
            gates.iter().map(|gate| gate.ql()).collect(),
            domain.d1,
        )
        .interpolate();
        let qrm = Evaluations::<F, D<F>>::from_vec_and_domain(
            gates.iter().map(|gate| gate.qr()).collect(),
            domain.d1,
        )
        .interpolate();
        let qom = Evaluations::<F, D<F>>::from_vec_and_domain(
            gates.iter().map(|gate| gate.qo()).collect(),
            domain.d1,
        )
        .interpolate();
        let qmm = Evaluations::<F, D<F>>::from_vec_and_domain(
            gates.iter().map(|gate| gate.qm()).collect(),
            domain.d1,
        )
        .interpolate();

        // compute poseidon constraint polynomials
        let psm = Evaluations::<F, D<F>>::from_vec_and_domain(
        ...
        let addm = Evaluations::<F, D<F>>::from_vec_and_domain(
            gates.iter().map(|gate| gate.add1()).collect(),
            domain.d1,
        )
        .interpolate();
        let mul1m = Evaluations::<F, D<F>>::from_vec_and_domain(
            gates.iter().map(|gate| gate.vbmul1()).collect(),
            domain.d1,
        )
        .interpolate();
        let mul2m = Evaluations::<F, D<F>>::from_vec_and_domain(
            gates.iter().map(|gate| gate.vbmul2()).collect(),
            domain.d1,
        )
        .interpolate();
        let emul1m = Evaluations::<F, D<F>>::from_vec_and_domain(
            gates.iter().map(|gate| gate.endomul1()).collect(),
            domain.d1,
        )
        .interpolate();
        let emul2m = Evaluations::<F, D<F>>::from_vec_and_domain(
            gates.iter().map(|gate| gate.endomul2()).collect(),
            domain.d1,
        )
        .interpolate();
        let emul3m = Evaluations::<F, D<F>>::from_vec_and_domain(
            gates.iter().map(|gate| gate.endomul3()).collect(),
            domain.d1,
        )
        .interpolate();
        ...
        Some(ConstraintSystem {
            ...
            qom,
            qmm,
            qc: Evaluations::<F, D<F>>::from_vec_and_domain(
                gates.iter().map(|gate| gate.qc()).collect(),
                domain.d1,
            )
            .interpolate(),
            ...
            rcm: array_init(|i| {
                Evaluations::<F, D<F>>::from_vec_and_domain(
                    gates.iter().map(|gate| gate.rc()[i]).collect(),
                    domain.d1,
                )
                .interpolate()
            }),
            ...
            l04: DensePolynomial::from_coefficients_slice(&[F::one()])
                .evaluate_over_domain_by_ref(domain.d4),
            l08: DensePolynomial::from_coefficients_slice(&[F::one()])
                .evaluate_over_domain_by_ref(domain.d8),
            l1: DensePolynomial::from_coefficients_slice(&[F::zero(), F::one()])
                .evaluate_over_domain_by_ref(domain.d8),
    ...
    pub fn verify(&self, witness: &Vec<F>) -> bool {
        if witness.len() != 3 * self.domain.d1.size() {
            return false;
        }
