...
impl<F: FftField + SquareRootField> ConstraintSystem<F>
{
    ...
    {
        if self.mulm.is_zero() {return (self.zero4.clone(), self.zero8.clone())}
        ...
        [
            ...
            // (2*xp + xt – s1^2) * (s1 + s2) = 2*yp
            &(&(&(&this4[4].scale(F::from(2 as u64)) + &this4[0]) - &this4[9].pow(2)) * &(&this4[9] + &this4[10])) - &this4[5].scale(F::from(2 as u64)),
            // (xp – xr) * s2 = yr + yp
            &(&(&(&this4[4] - &this4[7]) * &this4[10]) - &this4[8]) - &this4[5],

            // (xr - xt) * s3 = yr – (2b2-1)*yt
            &(&(&(&this4[7] - &this4[0]) * &this4[12]) - &this4[8]) + &(&this4[1] * &(&this4[14].scale(F::from(2 as u64)) - &self.l04)),
            // S3^2 – s4^2 = xt - xs
            &(&(&this4[12].pow(2) - &this4[13].pow(2)) - &this4[0]) + &this4[2],
            // (2*xr + xt – s3^2) * (s3 + s4) = 2*yr
            &(&(&(&this4[7].scale(F::from(2 as u64)) + &this4[0]) - &this4[12].pow(2)) * &(&this4[12] + &this4[13])) - &this4[8].scale(F::from(2 as u64)),
            // (xr – xs) * s4 = ys + yr
        ...
        [
            ...
            &(&(&(&this8[0] - &next8[4]) * &next8[11]) - &(&(&next8[12].scale(F::from(2 as u64)) - &self.l04) * &this8[1])) + &next8[5],
            // (2*xp – s1^2 + xt) * ((xp – xr) * s1 + yr + yp) = (xp – xr) * 2*yp
            &(&(&(&next8[4].scale(F::from(2 as u64)) - &next8[11].pow(2)) + &this8[0]) * &(&(&(xpmxr * &next8[11]) + &next8[8]) + &next8[5]))
            -
            &(xpmxr * &next8[5].scale(F::from(2 as u64))),
            // (yr + yp)^2 = (xp – xr)^2 * (s1^2 – xt + xr)
            &(&next8[8] + &next8[5]).pow(2) - &(&xpmxr.pow(2) * &(&(&next8[11].pow(2) - &this8[0]) + &next8[7])),

            // (xt - xr) * s3 = (2b2-1)*yt - yr
            &(&(&(&this8[0] - &next8[7]) * &next8[13]) - &(&(&next8[14].scale(F::from(2 as u64)) - &self.l04) * &this8[1])) + &next8[8],
            // (2*xr – s3^2 + xt) * ((xr – xv) * s3 + yv + yr) = (xr – xv) * 2*yr
            &(&(&(&next8[7].scale(F::from(2 as u64)) - &next8[13].pow(2)) + &this8[0]) * &(&(&(xrmxv * &next8[13]) + &next8[10]) + &next8[8]))
            -
            &(xrmxv * &next8[8].scale(F::from(2 as u64))),
            // (yv + yr)^2 = (xr – xv)^2 * (s3^2 – xt + xv)
            &(&next8[10] + &next8[8]).pow(2) - &(&xrmxv.pow(2) * &(&(&next8[13].pow(2) - &this8[0]) + &next8[9])),

            // (xt - xv) * s5 = (2b3-1)*yt - yv
            &(&(&(&this8[0] - &next8[9]) * &next8[0]) - &(&(&next8[1].scale(F::from(2 as u64)) - &self.l04) * &this8[1])) + &next8[10],
            // (2*xv – s5^2 + xt) * ((xv – xs) * s5 + ys + yv) = (xv – xs) * 2*yv
            &(&(&(&next8[9].scale(F::from(2 as u64)) - &next8[0].pow(2)) + &this8[0]) * &(&(&(xvmxs * &next8[0]) + &next8[3]) + &next8[10]))
            -
            &(xvmxs * &next8[10].scale(F::from(2 as u64))),
            // (ys + yv)^2 = (xv – xs)^2 * (s5^2 – xt + xs)
            &(&next8[3] + &next8[10]).pow(2) - &(&xvmxs.pow(2) * &(&(&next8[0].pow(2) - &this8[0]) + &next8[2])),
        ];
        (
    ...
    pub fn vbmul_scalars(evals: &Vec<ProofEvaluations<F>>, alpha: &[F]) -> F
    {
        ...
        [
            ...
            // (2*xp – s1^2 + xt) * ((xp – xr) * s1 + yr + yp) = (xp – xr) * 2*yp
            (((next[4].double() - &next[11].square()) + &this[0]) * &(((xpmxr * &next[11]) + &next[8]) + &next[5]))
            -
            &(xpmxr * &next[5].double()),
            // (yr + yp)^2 = (xp – xr)^2 * (s1^2 – xt + xr)
            (next[8] + &next[5]).square() - &(xpmxr.square() * &((next[11].square() - &this[0]) + &next[7])),

            // (xt - xr) * s3 = (2b2-1)*yt - yr
            (((this[0] - &next[7]) * &next[13]) - &((next[14].double() - &F::one()) * &this[1])) + &next[8],
            // (2*xr – s3^2 + xt) * ((xr – xv) * s3 + yv + yr) = (xr – xv) * 2*yr
            (((next[7].double() - &next[13].square()) + &this[0]) * &(((xrmxv * &next[13]) + &next[10]) + &next[8]))
            -
            &(xrmxv * &next[8].double()),
            // (yv + yr)^2 = (xr – xv)^2 * (s3^2 – xt + xv)
            (next[10] + &next[8]).square() - &(xrmxv.square() * &((next[13].square() - &this[0]) + &next[9])),

            // (xt - xv) * s5 = (2b3-1)*yt - yv
            (((this[0] - &next[9]) * &next[0]) - &((next[1].double() - &F::one()) * &this[1])) + &next[10],
            // (2*xv – s5^2 + xt) * ((xv – xs) * s5 + ys + yv) = (xv – xs) * 2*yv
            (((next[9].double() - &next[0].square()) + &this[0]) * &(((xvmxs * &next[0]) + &next[3]) + &next[10]))
            -
            &(xvmxs * &next[10].double()),
            // (ys + yv)^2 = (xv – xs)^2 * (s5^2 – xt + xs)
            (next[3] + &next[10]).square() - &(xvmxs.square() * &((next[0].square() - &this[0]) + &next[2])),
        ];
