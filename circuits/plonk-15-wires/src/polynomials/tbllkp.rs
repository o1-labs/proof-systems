...
impl<F: FftField + SquareRootField> ConstraintSystem<F>
{
    ...
    ) -> Result<(E<F, D<F>>, DP<F>), ProofError>
    {
        ...
        let h2w = DP::from_coefficients_slice(&lkppolys.h2.coeffs.iter().
            zip(self.cs.sid.iter()).map(|(z, w)| *z * w).collect::<Vec<_>>());
        let (bnd2, res) =
            DenseOrSparsePolynomial::divide_with_q_and_r(
    ...
    ) -> LookupEvals<F>
    {
        let n = self.cs.domain.d1.size as usize;
        // get lookup values
        let lw = witness[COLUMNS-1].iter().take(n-1).zip(self.cs.gates.iter()).
    ...
    ) -> Result<DP<F>, ProofError>
    {
        ...
        (
            |j| z[j+1] =
                (gammabeta1 + lkpevl.h1.evals[j] + (oracles.beta * lkpevl.h1.evals[j+1])) *
        ...
        (
            ...
            {
                let x = z[j];
                z[j+1] *= &(x * beta1 * (oracles.gamma + lkpevl.lw.evals[j]) *
        ...
        Ok(&lkpevl.l.interpolate_by_ref() + &DP::rand(2, rng).mul_by_vanishing_poly(self.cs.domain.d1))
    }
}
