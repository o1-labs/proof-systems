...
use oracle::{poseidon::ArithmeticSpongeParams, utils::EvalUtils};
...
pub struct ConstraintSystem<F: FftField> {
    ...
    pub sigmam: [DP<F>; wires::PERMUTS], // permutation polynomial array
    pub zkpm: DP<F>,                     // zero-knowledge polynomial

    // generic constraint selector polynomials
    pub qwm: [DP<F>; wires::GENERICS], // linear wire constraint polynomial
    pub qmm: DP<F>,                    // multiplication polynomial
    pub qc: DP<F>,                     // constant wire polynomial

    // poseidon selector polynomials
    pub rcm: [[DP<F>; poseidon::SPONGE_WIDTH]; poseidon::ROUNDS_PER_ROW], // round constant polynomials
    ...
    // generic constraint selector polynomials
    pub qwl: [E<F, D<F>>; wires::GENERICS], // left input wire polynomial over domain.d4
    pub qml: E<F, D<F>>,                    // multiplication evaluations over domain.d4

    // permutation polynomials
    pub sigmal1: [Vec<F>; wires::PERMUTS], // permutation polynomial array evaluations over domain d1
    pub sigmal8: [E<F, D<F>>; wires::PERMUTS], // permutation polynomial array evaluations over domain d8
    ...
    pub shift: [F; wires::PERMUTS], // wire coordinate shifts
...
impl<F: FftField + SquareRootField> ConstraintSystem<F> {
    pub fn create(
        ...
        let shift = Self::sample_shifts(&domain.d1, wires::PERMUTS - 1);
        let shift: [F; wires::PERMUTS] =
            array_init(|i| if i == 0 { F::one() } else { shift[i - 1] });
        ...
        let s: [std::vec::Vec<F>; wires::PERMUTS] =
        ...
        gates.iter().enumerate().for_each(|(i, _)| {
            (0..wires::PERMUTS).for_each(|j| {
                let wire = gates[i].wires[j];
                sigmal1[j][i] = s[wire.col][wire.row]
            })
        });
        let sigmam: [DP<F>; wires::PERMUTS] = array_init(|i| {
        ...
        let qwm: [DP<F>; wires::GENERICS] = array_init(|i| {
            E::<F, D<F>>::from_vec_and_domain(
                gates
                    .iter()
                    .map(|gate| {
                        if gate.typ == GateType::Generic {
                            gate.c[wires::WIRES[i]]
                        } else {
                            F::zero()
                        }
                    })
    ...
    pub fn evaluate(&self, w: &[DP<F>; wires::COLUMNS], z: &DP<F>) -> WitnessOverDomains<F> {
        // compute shifted witness polynomials
        let w8: [E<F, D<F>>; wires::COLUMNS] =
            array_init(|i| w[i].evaluate_over_domain_by_ref(self.domain.d8));
        let z8 = z.evaluate_over_domain_by_ref(self.domain.d8);

        let w4: [E<F, D<F>>; wires::COLUMNS] = array_init(|i| {
            E::<F, D<F>>::from_vec_and_domain(
